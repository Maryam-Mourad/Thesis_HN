...
infile = TFile( '/eos/user/m/mmourad/HN_simulation/trees/gamma9_tree.root' )
...
h_theta_HN = TH1D("h_theta_HN","angle between visible HN and #gamma(B);#theta(visible HN, #gamma(B))", 40,0,pi)
...
#h_energy_mu3 = TH1D("h_energy_mu3","Energy of #mu_{3};E_{#mu_{3}} [GeV]",10, 0,1000)
...
#Dalitz plot for HN
h_dalitz_HN = TH2D("h_dalitz_HN ","Dalitzplot for B+ -> #mu+#mu+#mu-;m^{2}_{#mu_{1}#mu_{3}} [GeV];m^{2}_{#mu_{2}#mu_{3}} [GeV]" ,45 ,0 ,15, 45, 0 , 15)
...
h_m_mu1mu2 = TH1D("h_m_mu1mu2", "Invariant mass of mu1 and mu2; invariant mass of #mu_{1} and #mu_{2} [GeV]", 40, 0, 6)
h_m_mu1mu3 = TH1D("h_m_mu1mu3", "Invariant mass of mu1 and mu3; invariant mass of #mu_{1} and #mu_{3} [GeV]", 40, 0, 6)
h_m_mu2mu3 = TH1D("h_m_mu2mu3", "Invariant mass of mu2 and mu3; invariant mass of #mu_{2} and #mu_{3} [GeV]", 60, 0, 6)
...
for event in range( entries ):
 ...
 weight = FakeFactorK(mu1.P())
 
 #neutrino = B - mu1 - mu2 - mu3  
 #fill histograms
 h_pt_B.Fill(B.Pt(),weight)
 h_energy_B.Fill(B.E(),weight)
 h_pt_HN.Fill(-1,weight)
 h_energy_HN.Fill(-1,weight)
 h_energy_mu3.Fill(mu3.E(),weight)
 h_energy_mu1.Fill(mu1.E(),weight)
 h_energy_mu2.Fill(mu2.E(),weight)
 h_pt_mu1.Fill(mu1.Pt(),weight)
 h_pt_mu2.Fill(mu2.Pt(),weight)
 h_pt_mu1mu2.Fill((mu1+mu2).Pt(),weight)
 h_pt_mu1mu3.Fill((mu1+mu3).Pt(),weight)
 h_pt_mu2mu3.Fill((mu2+mu3).Pt(),weight)
 h_pt_mu3.Fill(mu3.Pt(),weight)
 h_pt_neutrino.Fill(neutrino.Pt(),weight)
 h_phi_mu1.Fill(mu1.Phi(),weight)
 h_phi_mu2.Fill(mu2.Phi(),weight)
 h_phi_mu3.Fill(mu3.Phi(),weight)
 h_phi_neutrino.Fill(neutrino.Phi(),weight)
 h_deltaphi_mu1mu2.Fill(abs(mu1.DeltaPhi(mu2)),weight)
 h_deltaphi_mu1mu3.Fill(abs(mu1.DeltaPhi(mu3)),weight)
 h_deltaphi_mu2mu3.Fill(abs(mu2.DeltaPhi(mu3)),weight)
 h_deltaeta_mu1mu2.Fill(abs(mu1.Eta()-mu2.Eta()),weight)
 h_deltaeta_mu1mu3.Fill(abs(mu1.Eta()-mu3.Eta()),weight)
 h_deltaeta_mu2mu3.Fill(abs(mu2.Eta()-mu3.Eta()),weight)
 h_deltaR_mu1mu2.Fill(abs(mu1.DeltaR(mu2)),weight) 
 h_deltaR_mu1mu3.Fill(abs(mu1.DeltaR(mu3)),weight) 
 h_deltaR_mu2mu3.Fill(abs(mu2.DeltaR(mu3)),weight) 
 
 
 
 
 visible_B_boost = (mu1+mu2+mu3).BoostVector()
 boosted_HN_BRF = mu2+mu3
 boosted_HN_BRF.Boost(-visible_B_boost)
 h_theta_HN.Fill(abs(boosted_HN_BRF.Angle((mu1+mu2+mu3).Vect())),weight)
 ...
 h_energy_mu3_HNRF.Fill(boosted_mu3_HNRF.E(),weight)
 ...
 HN_FDt = -1
 HN_FD = -1
 h_FDx_HN.Fill(HN_FDx,weight)
 h_FDy_HN.Fill(HN_FDy,weight)
 h_FDt_HN.Fill(HN_FDt,weight)
 h_FDz_HN.Fill(HN_FDz,weight)
 h_FD_HN.Fill(HN_FD,weight)
 ...
 FD_mu1_mu3 = math.sqrt(FDx_mu1_mu3**2 + FDy_mu1_mu3**2 + FDz_mu1_mu3**2)
 h_FD_mu1_mu3.Fill(FD_mu1_mu3,weight)
 ...
 h_mu1mu3_vtxZ.Fill(mu1mu3_vtxZ,weight)
 h_mu1mu3_vtxT.Fill(mu1mu3_vtxT,weight)
 h_mu1mu3_vtx.Fill(mu1mu3_vtx,weight)
 ...
 #invariant mass
 m_Jpsi = (mu1+mu2).M()
 h_m_Jpsi.Fill(m_Jpsi,weight)
 h_m_mu1mu2.Fill((mu1+mu2).M(),weight)
 h_m_mu1mu3.Fill((mu1+mu3).M(),weight)
 h_m_mu2mu3.Fill((mu2+mu3).M(),weight)
 msquared_mu1mu3 = (mu1 + mu3).M() **2
 msquared_mu2mu3 = (mu2 + mu3).M() **2
 h_dalitz_HN.Fill(msquared_mu1mu3, msquared_mu2mu3,weight)
 m_HN = (mu2 + mu3).M()
 h_m_HN.Fill(m_HN,weight)
 m_HNprime = (mu2 + mu3 + neutrino).M()
 h_m_HNprime.Fill(m_HNprime)
 h_m_B.Fill((mu1+mu2+mu3).M(),weight)
 #2d plots
