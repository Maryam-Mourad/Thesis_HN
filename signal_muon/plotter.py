...
for massID in range(len(mass_list)):
    ...
    if(h_pt_mu3.GetMaximum()>maxY or maxY==-1):
      maxY = h_pt_mu3.GetMaximum()
    if(h_pt_mu3.GetMinimum()<minY or minY==-1):
      minY = h_pt_mu3.GetMinimum()
...
for massID in range(len(mass_list)):
    ...
    if (massID == 0):
      h_pt_mu3.GetYaxis().SetRangeUser(minY,maxY)
