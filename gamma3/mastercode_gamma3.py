...
tau_energy_thr = 30 #30
muon_pt_thr = 5 #5

cutflow_labels = ["generated","LHCb geo","mu1 trig","mu2 trig", "tau trig", "J/#psi mass", "B_{c} mass"]
...
infile = TFile( '/eos/user/m/mmourad/HN_simulation/trees/gamma3_tree.root' )
...
h_cutflow = TH1D("h_cutflow","#mu#mu#tau",len(cutflow_labels),0,len(cutflow_labels))
...
h_mu1Tau_vtxZ = TH1D("h_mu1Tau_vtxZ","vertex distance of mu1 and tau in Z;vertex distance of mu1 and tau in Z [mm]",50,0,10)
h_mu1Tau_vtxT = TH1D("h_mu1Tau_vtxT","transverse vertex distance of mu1 and tau;transverse vertex distance of mu1 and tau [mm]",50,0,10)
h_mu1Tau_vtx = TH1D("h_mu1Tau_vtx","total vertex distance of mu1 and tau;total vertex distance of mu1 and tau [mm]",50,0,10)
...
h_pt_tau = TH1D("h_pt_tau","p_{T} of #tau;p_{T} [GeV]",50,0,100)
h_pt_HN = TH1D("h_pt_HN","p_{T} of HN;p_{T} [GeV]",50,0,100)
h_pt_B = TH1D("h_pt_B","p_{T} of B_{c};p_{T} [GeV]",50,0,100)
h_pt_neutrino = TH1D("h_pt_neutrino","p_{T} of #nu_{#tau};p_{T} [GeV]",50,0,30)
...
h_energy_tau = TH1D("h_energy_tau","Energy of #tau;E_{#tau} [GeV]",100, tau_energy_thr,1000)
...
#Dalitz plot for HN
h_dalitz_HN = TH2D(" h_dalitz_HN ","Dalitzplot for B_{c}+ -> #mu+#mu+#tau-;m^{2}_{#mu_{1}#tau} [GeV];m^{2}_{#mu_{2}#tau} [GeV]" ,100 ,0 ,50, 100, 0 , 10)
...
for event in range( entries ):
 ...
 mu1Tau_vtxX = tree.mu1_origX - tree.mu2_origX
 mu1Tau_vtxY = tree.mu1_origY - tree.mu2_origY
 mu1Tau_vtxZ = tree.mu1_origZ - tree.mu2_origZ
 mu1Tau_vtxT = sqrt(mu1Tau_vtxX**2 + mu1Tau_vtxY**2)
 mu1Tau_vtx = sqrt(mu1Tau_vtxX**2 + mu1Tau_vtxY**2 + mu1Tau_vtxZ**2)
 
 h_mu1Tau_vtxZ.Fill(mu1Tau_vtxZ)
 h_mu1Tau_vtxT.Fill(mu1Tau_vtxT)
 h_mu1Tau_vtx.Fill(mu1Tau_vtx)
 ...
 h_dalitz_HN.Fill(msquared_mu1tau, msquared_mu2tau)
 m_HN = (mu2 + tau).M()
 h_m_HN.Fill(m_HN)
 m_HNprime = (mu2 + tau + neutrino).M()
 h_m_HNprime.Fill(m_HNprime)
 h_m_B.Fill((mu1+mu2+tau).M())
 #2d plots
...
c_mu1Tau_vtxZ = TCanvas("c_mu1Tau_vtxZ","canvas title", 800, 800) 
c_mu1Tau_vtxZ.SetLogy(1)
h_mu1Tau_vtxZ.Draw()
c_mu1Tau_vtxZ.SaveAs("plots/mu1Tau_vtxZ.png")

c_mu1Tau_vtxT = TCanvas("c_mu1Tau_vtxT","canvas title", 800, 800) 
c_mu1Tau_vtxT.SetLogy(1)
h_mu1Tau_vtxT.Draw()
c_mu1Tau_vtxT.SaveAs("plots/mu1Tau_vtxT.png")

c_mu1Tau_vtx = TCanvas("c_mu1Tau_vtx","canvas title", 800, 800) 
c_mu1Tau_vtx.SetLogy(1)
h_mu1Tau_vtx.Draw()
c_mu1Tau_vtx.SaveAs("plots/mu1Tau_vtx.png")
