...
do
    ...
    if [ -r HN${masses[$massPoint]} ]; then
        ...
        cp mastercode_signal_pion.py HN${masses[$massPoint]}/
    else
        mkdir HN${masses[$massPoint]}
        echo HN directory created.
        mkdir HN${masses[$massPoint]}/plots
        cp signal_pion.config HN${masses[$massPoint]}/signal_pion_mass${masses[$massPoint]}.config
        cp signal_pion.decay HN${masses[$massPoint]}/signal_pion_mass${masses[$massPoint]}.decay
        cp mastercode_signal_pion.py HN${masses[$massPoint]}/
    ...
    if [ $generation = generate ]; then
        ...
        $RAPIDSIM_ROOT/build/src/RapidSim.exe $RAPIDSIM_ROOT/HN_simulation/signal_pion/HN${masses[$massPoint]}/signal_pion_mass${masses[$massPoint]} $nEvents 1
        mv signal_pion_mass${masses[$massPoint]}_tree.root /eos/user/m/mmourad/HN_simulation/trees/
    else
    ...
    python3 mastercode_signal_pion.py ${masses[$massPoint]} $massPoint $nEvents
...
hadd -f histograms_pion.root histograms.root ../gamma4/histograms.root
cd ..
python3 plotter_pion.py ${masses[@]}
