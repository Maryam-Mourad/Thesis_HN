...
for massID in range(len(mass_list)):
    ...
    if(h_pt_pi.GetMaximum()>maxY or maxY==-1):
      maxY = h_pt_pi.GetMaximum()
    if(h_pt_pi.GetMinimum()<minY or minY==-1):
      minY = h_pt_pi.GetMinimum()
...
for massID in range(len(mass_list)):
    ...
    if (massID == 0):
      h_pt_pi.GetYaxis().SetRangeUser(minY,maxY)
    h_pt_pi.Draw("hesame")
legend.Draw()
