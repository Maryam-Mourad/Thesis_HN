...
backID = [4]
background_processes = ["B_{c}#rightarrowJ/#psi+#pi"]

hists = {'cutflow':1,'FD_HN':1,'FDx_HN':0,'FDy_HN':0,'FDt_HN':1,'FDz_HN':1,'FD_HN_norm':1,
        'energy_B':1,'energy_HN':1,'energy_pi':1,'energy_mu1':1,'energy_mu2':1,
        'pt_B':1,'pt_HN':1,'pt_mu1':1,'pt_mu2':1,'pt_pi':1,'pt_neutrino':1,'pt_mu1mu2':1,
        'phi_mu1':0,'phi_mu2':0,'phi_pi':0,'phi_neutrino':0,
        'deltaphi_mu1mu2':0,'deltaphi_mu1pi':0,'deltaphi_mu2pi':0,'deltaR_mu2pi':0,
        'm_HN':1,'m_HNprime':1,'m_Jpsi':1,'m_B':1}


merged_file = TFile("signal_pion/histograms_pion.root","READ")
...
for h in range(len(hists)):
    ...
    for b in backID:
        ...
        temp.Draw("hesame")
    legend.Draw()
    c_temp.SaveAs("plots_pion/"+hName+".png")
